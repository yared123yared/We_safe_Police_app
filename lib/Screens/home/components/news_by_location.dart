...
class NewsByLocation extends StatefulWidget {
  const NewsByLocation({Key? key}) : super(key: key);

  @override
  State<NewsByLocation> createState() => _NewsByLocationState();
}

class _NewsByLocationState extends State<NewsByLocation> {
  int _currentIndex = 0;
  @override
  Widget build(BuildContext context) {
    ...
    return BlocProvider<NewsBloc>(
          create: ((context) =>  NewsBloc(respository: _newsRespository)..add(FetchNewByLocation())),
        
      child: BlocBuilder<NewsBloc, NewsState>(
        builder: (context, state) {
          if (state is NewsInitial || state is NewsLocationLoading) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          } else if (state is NewsLocationLoaded) {
            ...
            NewsLocationError _errorState = state as NewsLocationError;
            return Center(
